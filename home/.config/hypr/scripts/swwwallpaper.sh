...
# Based on https://github.com/prasanthrangan/hyprdots/blob/be698ac275aac78951c251cfdff74d9d65964e7e/Configs/.config/hypr/scripts/swwwallpaper.sh
...
Wall_Update()
{
    ...
    wait
    awk -F '|' -v thm="${curTheme}" -v wal="${x_update}" '{OFS=FS} {if($2==thm)$NF=wal;print$0}' "${ThemeCtl}" > ${cacheDir}/tmpwall && mv -f ${cacheDir}/tmpwall "${ThemeCtl}"
...
mapfile -d '' Wallist < <(find -L ${wallPath} -type f \( -iname "*.jpg" -o -iname "*.jpeg" -o -iname "*.png" \) -print0 | sort -z)
